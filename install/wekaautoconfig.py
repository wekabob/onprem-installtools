...
class WekaHost:
    ...
    def __init__(self, hostname):
        ...
        # make mgmt_ip a list so we can support HA?
...
# only ask if we have both ETH and IB dp interfaces active?
...
for hostname, host in sorted( cluster.hosts.items() ):
    ...
    for name, ipobj in sorted( host.ipifs.iteritems() ): # check them all
...
for hostname, host in sorted( cluster.hosts.items() ):
    ...
    if len( host.weka_mgmt_ip ) > 2:
        print "Error: host " + hostname + " has more than 2 possible dataplane interfaces."
...
# no support for default-net anymore in this tool
# default-net
